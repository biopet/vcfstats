...
object VcfStatsSpark extends ToolCommand[Args] {
  ...
  def mainFromArgs(cmdArgs: Args): Unit = {
    ...
    try {
      ...
      val shortContigs = sc.broadcast(regions.value.groupBy(_.chr).filter(_._2.length == 1).values.flatten.toList)
      val longContigs = regions.value.groupBy(_.chr).filter(_._2.length != 1).map(x => x._1 -> sc.broadcast(x._2)).toList

      val contigsProcess = processContig(shortContigs, cmdArgsBroadcast, header, None) ::
        (for ((contig, regions) <- longContigs) yield {
          processContig(regions, cmdArgsBroadcast, header, Some(contig))
        })

      val generalStats = totalGeneralStats(contigsProcess.flatMap(_.generalStats), cmdArgsBroadcast)
      val genotypeStats = totalGenotypeStats(contigsProcess.flatMap(_.genotypeStats), cmdArgsBroadcast, header)
      val sampleDistributions = totalSampleDistributions(contigsProcess.flatMap(_.sampleDistributions), cmdArgsBroadcast)
      val sampleCompare = totalSampleCompare(contigsProcess.flatMap(_.sampleCompare), cmdArgsBroadcast, header)
      val infoFieldCounts = totalContigInfoFieldCounts(cmdArgs.infoTags.map(x => x -> contigsProcess.flatMap(_.infoFieldCounts.get(x))).toMap, header, cmdArgsBroadcast)
      val genotypeFieldCounts = totalContigGenotypeFieldCounts(cmdArgs.genotypeTags.map(x => x -> contigsProcess.flatMap(_.genotypeFieldCounts.get(x))).toMap, header, cmdArgsBroadcast)

      logger.info("Done submitting jobs")

      Await.result(Future.sequence(contigsProcess.map(_.contigFuture)), Duration.Inf)
      Await.result(Future.sequence(List(generalStats, genotypeStats, sampleDistributions, sampleCompare, infoFieldCounts, genotypeFieldCounts).flatten), Duration.Inf)
    } finally {
      sc.stop()
    }
    logger.info("Done")
  ...
  def processContig(regions: Broadcast[List[BedRecord]],
    ...
    logger.info(s"$prefixMessage Vcf file in memory, submitting jobs to calculate stats")

    val sampleCompare = contigSampleCompare(vcfRecords, header, cmdArgs, regions, prefixMessage)
    val generalStats = contigGeneralStats(vcfRecords, cmdArgs, regions, prefixMessage)
    val genotypeStats = contigGenotypeStats(vcfRecords, header, cmdArgs, regions, prefixMessage)
    val sampleDistribution = contigSampleDistributions(vcfRecords, cmdArgs, regions, prefixMessage)

    logger.info("Submitting infoFieldCounts jobs")
    val infoCounts = contigInfoFieldCounts(vcfRecords, header, cmdArgs, regions, prefixMessage)

    logger.info("Submitting genotypeFieldCounts jobs")
    val genotypeCounts = contigGenotypeFieldCounts(vcfRecords, header, cmdArgs, regions, prefixMessage)
  ...
  def loadVcfFile(cmdArgs: Broadcast[Args],
    ...
    val vcfRecords = spark.vcf
      .loadRecords(cmdArgs.value.inputFile, regions, cmdArgs.value.binSize, cached = true)
  ...
  def contigGeneralStats(vcfRecords: RDD[VariantContext],
                         cmdArgs: Broadcast[Args],
                         regions: Broadcast[List[BedRecord]],
                         prefixMessage: String): (Option[FutureAction[Unit]], Option[RDD[(String, GeneralStats)]]) = {
    if (cmdArgs.value.skipGeneral) (None, None)
    else {
      vcfRecords.sparkContext.setJobGroup(s"$prefixMessage General stats", s"$prefixMessage General stats")
  ...
  def contigGenotypeStats(vcfRecords: RDD[VariantContext],
                          ...
                          cmdArgs: Broadcast[Args],
                          regions: Broadcast[List[BedRecord]],
                          prefixMessage: String): (Option[FutureAction[Unit]], Option[RDD[(String, GenotypeStats)]]) = {
    if (cmdArgs.value.skipGeneral) (None, None)
    else {
      vcfRecords.sparkContext.setJobGroup(s"$prefixMessage Genotype stats", s"$prefixMessage Genotype stats")
      val genotypeContig = spark.vcf.genotypeStats(vcfRecords, header, regions).cache()
      val contigFutures = if (!cmdArgs.value.notWriteContigStats) {
  ...
  def totalGenotypeStats(contigs: List[RDD[(String, GenotypeStats)]],
    ...
    if (contigs.nonEmpty) {
      ...
      Some(sc.union(emptyRdd :: contigs)
        .map("total" -> _._2)
        .reduceByKey(_ += _)
        .foreachAsync(
  ...
  def contigSampleDistributions(vcfRecords: RDD[VariantContext],
                                cmdArgs: Broadcast[Args],
                                regions: Broadcast[List[BedRecord]],
                                prefixMessage: String): (Option[FutureAction[Unit]], Option[RDD[(String, SampleDistributions)]]) = {
    if (cmdArgs.value.skipSampleDistributions) (None, None) else {
      vcfRecords.context.setJobGroup(s"$prefixMessage Sample Distributions", s"$prefixMessage Sample Distributions")
  ...
  def contigSampleCompare(vcfRecords: RDD[VariantContext],
                          ...
                          cmdArgs: Broadcast[Args],
                          regions: Broadcast[List[BedRecord]],
                          prefixMessage: String): (Option[FutureAction[Unit]], Option[RDD[(String, SampleCompare)]]) = {
    if (cmdArgs.value.skipSampleCompare) (None, None) else {
      vcfRecords.sparkContext.setJobGroup(s"$prefixMessage Sample compare", s"$prefixMessage Sample compare")
  ...
  def contigInfoFieldCounts(vcfRecords: RDD[VariantContext],
                            header: Broadcast[VCFHeader],
                            cmdArgs: Broadcast[Args],
                            regions: Broadcast[List[BedRecord]], prefixMessage: String): Map[VcfField, (Option[Future[Unit]], RDD[(String, InfoFieldCounts)])] = {
    (for (tag <- cmdArgs.value.infoTags) yield tag -> {
      vcfRecords.sparkContext.setJobGroup(s"$prefixMessage Info field counts: $tag", s"$prefixMessage Info field counts: $tag")
      val tagBroadcast = vcfRecords.sparkContext.broadcast(tag)
      val contigCounts = spark.vcf.infoFieldCounts(vcfRecords, header, tagBroadcast, regions).cache()
      val contigFuture = if (!cmdArgs.value.notWriteContigStats) {
  ...
  def totalContigInfoFieldCounts(contigs: Map[VcfField, List[RDD[(String, InfoFieldCounts)]]],
    ...
    val futures: List[Future[Unit]] = (for ((field, counts) <- contigs) yield {
      if (counts.isEmpty) None else {
        val sc = counts.head.context
        sc.setJobGroup(s"Total Info field counts: $field", s"Total Info field counts: $field")
        val emptyRdd = sc.parallelize(
          List("total" -> new InfoFieldCounts(header.value.getInfoHeaderLine(field.key), field.method)))
        Some(sc.union(emptyRdd :: counts)
          .map("total" -> _._2)
          .reduceByKey(_ += _)
          .foreachAsync { x => x._2.writeHistogram(new File(cmdArgs.value.outputDir, s"info.$field.tsv"))})
      }
    }).flatten.toList
  ...
  def contigGenotypeFieldCounts(vcfRecords: RDD[VariantContext],
                            header: Broadcast[VCFHeader],
                            cmdArgs: Broadcast[Args],
                            regions: Broadcast[List[BedRecord]], prefixMessage: String): Map[VcfField, (Option[Future[Unit]], RDD[(String, GenotypeFieldCounts)])] = {
    (for (tag <- cmdArgs.value.genotypeTags) yield tag -> {
      vcfRecords.sparkContext.setJobGroup(s"$prefixMessage Genotype field counts: $tag", s"$prefixMessage Genotype field counts: $tag")
      val tagBroadcast = vcfRecords.sparkContext.broadcast(tag)
      val contigCounts = spark.vcf.genotypeFieldCounts(vcfRecords, header, tagBroadcast, regions).cache()
      val contigFuture = if (!cmdArgs.value.notWriteContigStats) {
        Some(contigCounts.foreachAsync {
          case (contig, counts) =>
            val file = new File(contigDir(cmdArgs.value.outputDir, contig),
  ...
  def totalContigGenotypeFieldCounts(contigs: Map[VcfField, List[RDD[(String, GenotypeFieldCounts)]]],
    ...
    val futures: List[Future[Unit]] = (for ((field, counts) <- contigs) yield {
      if (counts.isEmpty) None else {
        val sc = counts.head.context
        sc.setJobGroup(s"Total Genotype field counts: $field", s"Total Genotype field counts: $field")
        val emptyRdd = sc.parallelize(
          List("total" -> new GenotypeFieldCounts(header.value, header.value.getFormatHeaderLine(field.key), field.method)))
        Some(sc.union(emptyRdd :: counts)
          .map("total" -> _._2)
          .reduceByKey(_ += _)
          .foreachAsync { x =>
            x._2.writeToFile(new File(cmdArgs.value.outputDir, s"genotype.$field.tsv"))
          })
      }
    }).flatten.toList
    if (futures.nonEmpty) Some(Future.sequence(futures))
